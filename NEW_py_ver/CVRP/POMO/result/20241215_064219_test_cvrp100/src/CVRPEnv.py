...
class CVRPEnv:
    ...
    def step(self, selected, selected_beams, prob):
        ...
        # shape: (batch, pomo, beam)
        self.selected_node_list = torch.cat((self.selected_node_list[self.BATCH_IDX, self.POMO_IDX, selected_beams[self.BEAM_IDX]], self.current_node[:, :, :, None]), dim=3)
        # shape: (batch, pomo, beam, 0~)
        ...
        self.load = self.load[self.BATCH_IDX, self.POMO_IDX, selected_beams[self.BEAM_IDX]] - selected_demand
        self.load[self.at_the_depot] = 1 # refill loaded at the depot


        self.visited_ninf_flag = self.visited_ninf_flag[self.BATCH_IDX, self.POMO_IDX, selected_beams[self.BEAM_IDX]]
        self.visited_ninf_flag[self.BATCH_IDX, self.POMO_IDX, self.BEAM_IDX, selected] = float('-inf')
        # shape: (batch, pomo, beam,problem+1)
        ...
        self.logprob = self.logprob[self.BATCH_IDX, self.POMO_IDX, selected_beams] + torch.log(prob)
