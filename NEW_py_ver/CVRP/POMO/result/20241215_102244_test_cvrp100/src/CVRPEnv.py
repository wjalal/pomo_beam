...
class CVRPEnv:
    ...
    def step(self, selected, selected_beams, prob):
        ...
        # print('load: ', self.load[99, 61, 0])
        # print('node list: ', self.selected_node_list[99, 61, 0])

        self.ninf_mask = self.visited_ninf_flag.clone()
        round_error_epsilon = 0.00001
        demand_too_large = self.load[:, :, :, None] + round_error_epsilon < demand_list
        demand_too_large[:, :, :, 0] = False # depot node is not too heavy to return to
        # shape: (batch, pomo, beam, problem+1)
        self.ninf_mask[demand_too_large] = float('-inf')
        # shape: (batch, pomo, beam, problem+1)
        # print('inf: ', torch.where(self.ninf_mask[99, 61, 0] == float('-inf')))
