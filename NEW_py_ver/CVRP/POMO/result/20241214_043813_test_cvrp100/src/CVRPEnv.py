...
class CVRPEnv:
    ...
    def load_problems(self, batch_size, aug_factor=1):
        ...
        normalized_demand = nd / nd.max()  # Normalize between 0 and 1
        ...
        for (x, y), intensity in zip(nxy, normalized_demand):
            plt.scatter(x, y, color=(1, 0, 0, intensity), s=100, label="Node" if intensity == normalized_demand[0] else "")

        # Plot depots as squares
        for (x, y) in dxy:
            plt.scatter(x, y, color="blue", marker="s", s=100, label="Depot" if (x, y) == dxy[0].tolist() else "")
