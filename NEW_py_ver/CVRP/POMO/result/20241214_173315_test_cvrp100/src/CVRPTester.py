    ...
    def _test_one_batch(self, batch_size):
        ...
        ###############################################
            ...
            for k in range ( self.tester_params['env_count']):
                res_arr[k]['state'], res_arr[k]['reward'], res_arr[k]['done'] = env.pre_step()
            ...
            while not done:
                ...
                for k, env in enumerate(self.envs):
                    if not res_arr[k]['done']:
                        ...
                        selected, _ = self.model(res_arr[k]['state'])
                        res_arr[k]['state'], res_arr[k]['reward'], res_arr[k]['done'] = env.step(selected)

                # Check if all environments are done
                done = all(res['done'] for res in res_arr)

                if done:
                    # Find the entry in res_arr with the maximum reward
                    max_reward_entry = max(res_arr, key=lambda x: x['reward'])

                    # Set state and reward to those of the max reward entry
