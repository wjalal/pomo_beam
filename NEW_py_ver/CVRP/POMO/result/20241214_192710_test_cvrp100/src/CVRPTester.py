...
class CVRPTester:
    ...
    def _test_one_batch(self, batch_size):
        ...
        ###############################################
            ...
            gc.collect()
            N = self.tester_params['env_count']
            ...
            while not done:
                ...
                # self.envs = [copy.deepcopy(env) for _ in range(N) for env in self.envs]
                ...
                for k, env in enumerate(self.envs):
                    if not env.done:
                        ...
                        selected, prob = self.model(env.step_state)
                        print('eval type: ', self.model.model_params['eval_type'])
                        print('selected: ', selected.shape)
                        print('prob: ', prob.shape)
                        env.step(selected, prob)

                # Check if all environments are done
                done = all(env.done for env in self.envs)
