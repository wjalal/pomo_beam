...
class CVRPEnv:
    ...
    def load_problems(self, batch_size, aug_factor=1):
        ...
        # Normalize node demand for color intensity
            ...
            plt.scatter(
                dxy[:, 0], dxy[:, 1],
                color="blue", marker="s", s=100, label="Depot"
            )
    ...
    def step(self, selected):
        ...
        if done:
            ...
            for element in soln0:
                if element == 0:
                    soln0_path.append(self.dxy[0])  # Add the value from self.dxy[0] if the element is 0
                else:
                    soln0_path.append(self.nxy[element - 1])  # Add the value from self.nxy[element-1] otherwise
            print (soln0_path)
        else:
            reward = None
