...
class CVRPEnv:
    ...
    def load_problems(self, batch_size, aug_factor=1):
        ...
        # Normalize node demand for color intensity
            ...
            plt.scatter(
                dxy[:, 0], dxy[:, 1],
                color="blue", marker="s", s=20, label="Depot"
            )
    ...
    def step(self, selected):
        ...
        if done:
            ...
            # Plot the nodes
            plt.scatter(x_coords, y_coords, color="blue", s=50, zorder=5, label="Nodes")

            # Join the coordinates with straight lines
            plt.plot(x_coords, y_coords, color="black", linestyle="-", zorder=3, label="Path")

            # Mark the start and end of the path
            plt.scatter(x_coords[0], y_coords[0], color="green", s=100, zorder=6, label="Start")
            plt.scatter(x_coords[-1], y_coords[-1], color="red", s=100, zorder=6, label="End")
